...
def winemaker():
    while True:
        if running:
            ...
            pyautogui.moveTo(rnd.randint(700,980), rnd.randint(100,480), lag * 0.9 + 0.18)
            pyautogui.moveRel(rnd.randint(-40, 74), rnd.randint(-21,20), lag * 0.8 + 0.2)
            time.sleep(lag * 0.02 + 1)
            lag = (0.32 + rnd.random() + 2/27 * rnd.random())
            pyautogui.moveRel(rnd.randint(-40, 74), rnd.randint(2,60), lag * 1.2 + 1) # random movement
            time.sleep(lag * 0.02 + 0.05)
            lag = (0.43 + rnd.random() + 2/27 * rnd.random())
            ## ~~~ Random mouse movement ~~~~ ####
            # Begin opening bank: 
            pyautogui.moveTo(rnd.randint(880,900), rnd.randint(220,280), lag) # move mouse to banker.
            ...
            pyautogui.moveTo(rnd.randint(1005,1030) ,rnd.randint(819, 834), lag *0.53 + 0.42) # Move to deposit inventory tab 
            time.sleep(rnd.random() * 13/272 + 0.1) #sleep
            mouse.click() # Deposit all inventory
            pyautogui.moveRel(rnd.randint(-40, 74), rnd.randint(-2,60), lag * 0.123 + 0.02) # random movement
            time.sleep(rnd.random() * 13/100) #sleep

            pyautogui.moveTo(rnd.randint(940,950), rnd.randint(166, 172), lag * 0.2 + 0.16) #move to KNIFE <---- 
            time.sleep((rnd.random() * 0.1 + rnd.random() * 13/120 + 0.05)) # Sleep
            mouse.click() # Get knife
            time.sleep(0.02 + 0.13 * rnd.random()) # sleep.
            pyautogui.moveRel(rnd.randint(-12, 12), rnd.randint(-6,12), lag * 0.15 + 0.1) # random movement
            time.sleep(lag*0.15 + 0.08) #sleep
            print ("Getting wood.")
            pyautogui.moveTo(rnd.randint(986, 1004), rnd.randint(166, 172), lag * 0.133 + 0.06) # move to wood
            ...
            pyautogui.moveRel(rnd.randint(-12, 5), rnd.randint(100, 107), lag * 0.12 + 0.12) #move mouse down to quantity of 14
            ...
            # WINE PROCESS BEGIN !
            ## ~~~ Random mouse movement ~~~~ ####
            pyautogui.moveRel(rnd.randint(-20,40), rnd.randint(-10,6), lag * 0.12 + 0.03) #Move around from current spot
            time.sleep(lag * 0.03 + 0.006) #Sleep
            lag = (0.33 + rnd.random() + 2/27 * rnd.random()) # Reset YRT
            ## ~~~ Random mouse movement ~~~~ ####
            pyautogui.moveTo(rnd.randint(1058, 1065), rnd.randint(64, 68), lag * 0.13 + 0.04) # X of the bank
            time.sleep(0.01 + rnd.random() *0.098)
            mouse.click() #exit bank
            pyautogui.moveRel(rnd.randint(-20, 12), rnd.randint(-6,12), lag * 0.3 + 0.031) # random movement
            time.sleep(rnd.random() * 0.0987 + 0.02) #sleep short
            print("Position of bank exit click:", pyautogui.position())
            ## ~~~ Random mouse movement ~~~~ ####
            pyautogui.moveRel(rnd.randint(-20,40), rnd.randint(100,200), lag * 0.13 + 0.05) #Move around from current spot
            pyautogui.moveRel(rnd.randint(-2, 12), rnd.randint(6,12), lag * 0.13 + 0.05) # random movement
            time.sleep(lag * 0.13 + 0.01) #Sleep
            lag = (1.5* rnd.random() + 2/27 * rnd.random()) # Reset YRT
            ## ~~~ Random mouse movement ~~~~ ####

            pyautogui.moveTo(rnd.randint(1709,1720), rnd.randint(752, 768), lag * 0.5 + 0.15) #use jug of water
            time.sleep(rnd.random() *0.5 + 0.07)
            mouse.click()
            pyautogui.moveTo(rnd.randint(1747, 1767), rnd.randint(754, 769), lag * 0.23 + 0.15) #use on grapes
            time.sleep(rnd.random() * 0.14 + 0.85)
            mouse.click()

            pyautogui.moveTo(rnd.randint(220, 240), rnd.randint(935, 985), lag * 0.5 + 1.26) #make bow!
            time.sleep(rnd.random() *0.1 + 0.013)
            mouse.click()
            bowcount = bowcount + 1

            print ("This is batch number:", bowcount)
            pyautogui.moveTo(rnd.randint(860,940), rnd.randint(190,400), lag * 0.4 + 0.23) # move mouse to banker.
