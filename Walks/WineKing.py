...
def winemaker():
    while True:
        if running:
            ...
            clx() #open up the bank.
            time.sleep(0.04 + 0.13 * rnd.random()) # sleep.

            pyautogui.moveTo(rnd.randint(1005,1030) ,rnd.randint(820, 831), rnd.random() *0.23 + 0.42) # Move to deposit inventory tab 
            time.sleep(rnd.random() * 13/27 + 0.33) #sleep
            clx() # Deposit all inventory
            time.sleep(rnd.random() * 13/272 + 0.12) #sleep
            print("Position of inv depo click:", pyautogui.position())
            pyautogui.moveRel(rnd.randint(-14, 7), rnd.randint(-11,12), rnd.random() * 0.123 + 0.1) # random movement
            time.sleep(rnd.random() * 0.007) #sleep
            pyautogui.moveTo(rnd.randint(940,950), rnd.randint(132, 149), rnd.random() * 0.2 + 0.23) #move to the grapes 
            time.sleep((rnd.random() * 13/120 + 0.05)) # Sleep
            Getitems()
            pyautogui.moveRel(rnd.randint(-12, 12), rnd.randint(-6, 2), rnd.random() * 0.08 + 0.0051) # random movement
            time.sleep(rnd.random()*0.15 + 0.08) #sleep
            print ("Getting water")
            pyautogui.moveTo(rnd.randint(986, 1004), rnd.randint(134, 149), rnd.random() * 0.133 + 0.12) # move to water jug
            ...
            Notbotting3()
            Xbank()
            pyautogui.moveRel(rnd.randint(-20, 12), rnd.randint(-6,12), rnd.random() * 0.1 + 0.071) # random movement
            #time.sleep(rnd.random() * 0.0987 + 0.02) #sleep short

            pyautogui.moveTo(rnd.randint(1758,1762), rnd.randint(865, 872), rnd.random() * 0.21 + 0.25) #use jug of water
            time.sleep(rnd.random() *0.15 + 0.07)
            clx()
            pyautogui.moveTo(rnd.randint(1795, 1800), rnd.randint(865, 872), rnd.random() * 0.23 + 0.05) #use on grapes
            ...
            pyautogui.moveTo(rnd.randint(269, 280), rnd.randint(955, 965), rnd.random() * 0.3 + 0.43) #make wine!
            time.sleep(rnd.random() *0.1 + 0.183)
            clx()
            time.sleep(rnd.random() *0.1 + 0.313)
            pyautogui.moveTo(rnd.randint(300, 877), rnd.randint(305, 788), rnd.random() * 0.38 + 2.0051) # random movement
            time.sleep(rnd.random() *0.1 + 0.013)
            pyautogui.moveRel(rnd.randint(-9, 8), rnd.randint(-8, 8), rnd.random() * 0.38 + 2.0051) # random movement
            winecount = winecount + 1
            winecount2 = winecount2 + 1
            print ("Smells great! This is batch number:", winecount)
            pyautogui.moveTo(rnd.randint(860,940), rnd.randint(190,400), rnd.random() * 0.4 + 0.23) # move mouse to banker.
            if (winecount2 % 76 == 0):
