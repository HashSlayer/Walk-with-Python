...
class GGui:
    ...
    def kill_bot(self):
        ...
        kAltright()  # Preform the kill operation by automatically pressing the right alt key (Function in A2Functions.py)
    ...
    def toggle_walk_button(self):
        ...
        with running_lock:  # Ensure thread-safe access to the 'running' variable
            if not running: # Start the bot if running is False and toggle_bot() is called
                ...
                sleepy(1, 0, 0)  # Sleep for countdown
                print("Starting in 2")
                sleepy(1, 0, 0)
                print("Starting in 1")
...
def walker(gui):
    while True:
        if running:
            ...
            # Sleep logic
            sleepy(click_interval, interval_variance/2, interval_variance/2) #Sleep for a random amount of time between click_interval +/- wait_upto
            if not running:
                break
