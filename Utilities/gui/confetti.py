...
def create_confetti(canvas):
    ...
    for _ in range(random.randint(30, 50)):  # Reduced number of confetti pieces
        ...
        color = random.choice(['red', 'blue', 'green', 'yellow', 'purple'])  # Reduced color options
        piece = canvas.create_rectangle(x, y, x+3, y+3, fill=color)  # Smaller pieces
        confetti.append(piece)
    return confetti

def update_confetti(canvas, confetti, start_time, duration=1):  # Reduced duration from 2 to 1
    current_time = time.time()
    canvas_width = canvas.winfo_width()
    canvas_height = canvas.winfo_height()
    
    if current_time - start_time < duration:
        # Update all pieces in a single batch
        for item in confetti:
            x_move = random.randint(-3, 3)  # Reduced movement range
        ...
        canvas.after(30, update_confetti, canvas, confetti, start_time)  # Changed from 60ms to 30ms
    else:
        # Clean up confetti
        for item in confetti:
            try:
